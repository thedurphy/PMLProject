...
###### Now we will remove columns that have close to zero variance.  Columns that have close to no variance off no relevant information when creating a predicitive model since their data is close to constant and our outcome is not.
...
After achieving an accuracy of ~99%, we are ready to apply it to the final **dataTest**.  The **dataTest** set was not used in anyway and the actual answers was achieved from the submission portion of the project
...
Success!  The prediciton was 100% accurate on the final Data set.